    ...
    sock.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
        if (connection === 'open') {
            connectionState = {
                ...
                qr: null,
                code: null,
                message: `Connecté avec succès à ${sock.user.id.split(':')[0]}`
            };
            isConnecting = false;
            ...
            if (reason === DisconnectReason.loggedOut) {
                connectionState = {
                    ...
                    qr: null,
                    code: null,
                    message: 'Déconnecté avec succès. Prêt pour un nouveau code.'
                };
            } else {
                connectionState = {
                    status: 'disconnected',
                    qr: null,
                    code: null,
                    message: 'La connexion a été fermée. Prêt pour un nouveau code.'
                };
            }
            isConnecting = false;
        ...
        // Demande du code de pairage (remplace .once)
        if (!pairingCodeRequested && connection !== 'close' && phoneNumber && !sock.user) {
            ...
            try {
                connectionState = {
                    ...
                    qr: null,
                    code: null,
                    message: 'Demande du code de pairage...'
                };
                ...
                connectionState = {
                    ...
                    qr: null,
                    code,
                    message: 'Code prêt. Entrez-le sur votre téléphone.'
                };
            } catch (error) {
