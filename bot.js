...
async function connectToWhatsApp(phoneNumber = null) {
    ...
    if (phoneNumber && !sock.user) {
        try {
            ...
            connectionState = { status: 'code_ready', qr: null, code: code, message: 'Code prêt. Entrez-le sur votre téléphone.' };
        } catch (error) {
...
app.get('/status', (req, res) => {
    ...
    if (response.status === 'code_ready' || response.status === 'connected') {
        // Ne pas renvoyer le code une fois qu'il a été utilisé ou que la connexion est établie
    }
    res.json(response);
});
